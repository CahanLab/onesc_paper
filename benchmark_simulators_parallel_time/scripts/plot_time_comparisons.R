...
grab_OneSC_time <- function(file_path) {
  myData = read.delim(file_path, header = FALSE)
...
for(temp_trial in folder_list) {
  ...
  OneSC_time_df = data.frame(matrix(nrow = length(edge_config_list), ncol = 5))
  colnames(OneSC_time_df) = c("num_nodes", "num_edges", "time", 'style', 'trial')
  ...
  for(temp_edge_config in edge_config_list) {
    ...
    if("OneSC_time.txt" %in% list.files(focus_dir)) {
      OneSC_time = grab_OneSC_time(file.path(focus_dir, "OneSC_time.txt"))
    } else { 
      OneSC_time = NA
    }
    
    OneSC_time_df[i, 'time'] = OneSC_time
    OneSC_time_df[i, 'style'] = 'OneSC'
    OneSC_time_df[i, 'num_nodes'] = num_nodes
    OneSC_time_df[i, 'num_edges'] = num_edges
    OneSC_time_df[i, 'trial'] = temp_trial
    i = i + 1
  }
  big_plot_df = rbind(big_plot_df, BoolODE2_time_df, BoolODE_time_df, OneSC_time_df)
}

# to add in the auto-regulation 
#OneSC_time_df$num_edges = OneSC_time_df$num_nodes + OneSC_time_df$num_edges
...
for(num_nodes in unique(OneSC_time_df$num_nodes)) {
