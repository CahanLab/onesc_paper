...
if os.path.isdir("../output/min_dtw") == False: 
    os.mkdir("../output/min_dtw")
...
for dataset in datasets: 
    ...
    for method in methods_list:
        ...
        for grn_style in ['density']:
            ...
            cross_corr_files = [x for x in cross_corr_files if "dtw" in x]
            ...
            for temp_lineage in np.unique(big_df['lineage_type']):
                ...
                for temp_sample_id in np.unique(lineage_df['sample_id']):
                    average_score = np.mean(lineage_df.loc[lineage_df['sample_id'] == temp_sample_id, 'dtw'])
            ...
            save_path = "../output/min_dtw/" + dataset + "/" + method + "/" + grn_style
            Path(save_path).mkdir( parents=True, exist_ok=True )
            top_df.to_csv(save_path + "/" + "min_dtw.csv")
