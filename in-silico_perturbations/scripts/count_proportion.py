...
input_path = "../output/simulation_perturbation"
data_types = os.listdir(input_path)
data_types = [x for x in data_types if 'DS_Store' not in x]

out_path = "../output/ss_proportion"
...
for data_type in data_types: 
    print(data_type)
    states_dict = dict()
    all_states_dict = pickle.load(open(os.path.join('../output/files_needed', data_type, 'states_dict.pickle'), "rb"))
    ss_states = list()
    ...
    for gene in all_genes:
        if gene == 'wild_type':
            sim_folder_path = '../../benchmark_network_inference_real/output/simulation_OneSC/' + data_type + "/run_OneSC/density/simulation/" 
            ...
            sim_folder_path = '../output/simulation_perturbation/' + data_type + "/" + perturb_type + "/" + gene + "/"
            sim_files = [x for x in os.listdir(sim_folder_path) if '.csv' in x]
            raw_cluster_files = cluster_simulation(sim_folder_path)
            for temp_index in raw_cluster_files.keys():
                if temp_index in states_dict.keys():
