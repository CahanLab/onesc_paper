...
calc_correct_auto_reg <- function(gs_grn, inferred_grn) {
  sub_grn = gs_grn[grepl('+', gs_grn$Type), ] # only get inhibition 
  sub_grn = sub_grn[gs_grn$TF == gs_grn$TG, ]
  ...
  inferred_grn_sub = inferred_grn_sub[inferred_grn_sub$TF == inferred_grn_sub$TG, ]
  
  correct_percent = length(intersect(inferred_grn_sub$TF, sub_grn$TF)) / length(sub_grn$TF)
  wrong_percent = length(setdiff(inferred_grn_sub$TF, sub_grn$TF)) / length(sub_grn$TF)
...
for(datatype in datatypes) { 
  ...
  for(method in methods) { 
    ...
    correct_outputs = calc_correct_auto_reg(gs_grn, inferred_grn)
...
p <- ggplot(big_df, aes(x=reorder(methods,-percent_correct), y=percent_correct)) + 
  geom_boxplot()+
  theme_bw() + 
  ylab("auto regulation - percent correct") + 
  xlab("methods") +
  ggtitle('GRNs selected using optimal F1') + 
  theme(axis.text.x = element_text(angle = 45, vjust = 0.9, hjust=1))

ggsave(filename = '../output/auto_reg/auto_reg_percent_correct.png', plot = p, width = 10, height = 5)

p <- ggplot(big_df, aes(x=reorder(methods,percent_error), y=percent_error)) + 
  geom_boxplot()+
  theme_bw() + 
  ylab("auto regulation - percent error") + 
  xlab("methods") +
  ggtitle('GRNs selected using optimal F1') + 
  theme(axis.text.x = element_text(angle = 45, vjust = 0.9, hjust=1))

ggsave(filename = '../output/auto_reg/auto_reg_percent_error.png', plot = p, width = 10, height = 5)
