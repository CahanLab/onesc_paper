...
out_path = "../output/alternate_path/myeloid_progenitors_full/ss_prop"
...
input_path = '../output/alternate_path'
prop_df = pd.DataFrame(data = 0, columns = ss_states, index = ['wild_type'])
all_genes = ['wild_type']
for gene in all_genes:
    if gene == 'wild_type':
        sim_folder_path = '../output/alternate_path/myeloid_progenitors_full/wildtype/'
        ...
        for temp_index in raw_cluster_files.keys():
            if temp_index in states_dict.keys():
                prop_df.loc[gene, states_dict[temp_index]] = prop_df.loc[gene, states_dict[temp_index]] + len(raw_cluster_files[temp_index])
            else:
                prop_df.loc[gene, 'other'] = prop_df.loc[gene, 'other'] + len(raw_cluster_files[temp_index])
